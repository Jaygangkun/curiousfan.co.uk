...
<div class="leave-tip mt-2 d-none" data-post="{{ $post->id }}">
    <form method="POST" action="{{ route('sendTip', ['post' => $post->id]) }}" name="tipFrm-{{ $post->id }}">
        ...
        <div class="col-6 col-sm-6 col-md-3">
            <input name="tipAmountForPost-{{ $post->id }}" type="number" class="form-control" placeholder="@lang('general.minTip') {{ opt('payment-settings.currency_symbol') . number_format($post->profile->minTip,2) }}" min="{{ $post->profile->minTip  }}" required>
        </div>
        <div class="col-6 col-sm-6 col-md-3">
