        ...
        ******/
        ...
        VerifyEmail::toMailUsing(function($notifiable){
            $verifyUrl = URL::temporarySignedRoute(
                    'verification.verify', Carbon::now()->addMinute(60), [
                        'id' => $notifiable->getKey(),
                        'hash' => sha1($notifiable->getEmailForVerification())
                    ]
            );
            $getEmailTemplate = EmailTemplate::findorFail(11);
            ...
            $emailBody = str_replace('{verify_url}', $verifyUrl, $emailBody);

            return (new EmailVerifyMail($verifyUrl,$emailSubject,$emailBody))
                ->to($notifiable->email);

            /*return(new MailMessage())
                ->subject('Please verify your email')
                ->line('Whatever Your Line')
                ->action('Verify Now', $verifyUrl);*/
        });
